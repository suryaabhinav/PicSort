...
def mock_pipeline_background():
    with patch("backend.api.app.run_pipeline_background") as mock:
...
def mock_move_artifact():
    with patch("backend.api.app.move_with_run_artifact") as mock:
...
@pytest.fixture(autouse=True)
def mock_logging():
    with patch("backend.api.app.RotatingFileHandler"), patch("backend.api.app.get_logger"):
        yield
